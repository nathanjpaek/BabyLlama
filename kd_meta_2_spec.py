...
PATH = Path("/orion/u/yam/BabyLlama")
MODEL_NAME = 'Meta-Student-2'
...
# Define the checkpoint directory and find the last checkpoint
checkpoint_dir = "/orion/u/yam/BabyLlama/models/Meta-Student-2/checkpoint-14120"
last_checkpoint = get_last_checkpoint(checkpoint_dir) if Path(checkpoint_dir).is_dir() else None

# Only resume from checkpoint, otherwise raise an error
if not last_checkpoint:
    raise ValueError(f"No checkpoint found in {checkpoint_dir}. Training cannot start from scratch.")
...
maml_trainer.train(resume_from_checkpoint=last_checkpoint)
